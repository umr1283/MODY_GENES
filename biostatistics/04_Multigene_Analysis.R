...
## nohup nice -15 Rscript 04_Multigene_Analysis.R CLASSIQUE &
## nohup nice -15 Rscript 04_Multigene_Analysis.R ACMGCUSTOM &
...
## 5/ 
##  recalculer l'association avec le DT2 (STRICT + ACMG) rassemblant les gènes "officiels" du MODY [analyse multi-genes CC_T2D61]
...
Group6 <- sort(names(Genes_Variants25))
Group6
length(Group6)
Group6_final <- Group6
...
Group7 <- sort(names(Genes_Variants24))
Group7
length(Group7)
Group7_final <- Group7
...
for (analyse_LARGE_STRICT in analyses_to_do) {
  ...
  for (gene in allgenes) {
    ...
    res_QC <- DO_QC(
      Params, covar_names,LARGEdefSNP, LARGEdefINDEL, STRICTdefSNP, STRICTdefINDEL, CustomFile, 
      QC, Phenotypes, all_genotype_data, all_annotation_data, Detail_Geno, all_quality_data, 
      qc_type = "first5", GroupeGene = GroupeGene
    )
    QC <- res_QC$QC
    ...
    if (ncol(all_genotype_data) == 1 | nrow(Phenotypes) == 0) {
      ...
      if (Params["Gene_Symbol"] %in% Group7_final) {
        Group7_final <- Group7_final[-grep(Params["Gene_Symbol"], Group7_finals)]
      }
    } else {
  ...
  everything_old <- tibble(
    Analysis = rep(analyse_LARGE_STRICT, 7), ## --here adapt 7 groups 
  ...
  for (igroup in 1:nrow(everything_qcs)) {
    ...
    if (VERSION == "ACMGCUSTOM") {
      ...
      QC12_customfile <- read_excel(
        path = ".//Docs/CUSTOM_files/custom_03102019.xlsx", 
        sheet = 1,
        col_names = TRUE, 
        col_types = "text"
      )
      ...
      Genotype <- Genotype[, c("IID", Detail_Geno$Variant)]
    ...
    if (VERSION == "ACMGCUSTOM") {
      ...
      ## varaint excluded ... 
      ## variants ##   # VARIANT TO KEEP 
      ## (intersect between Custom file and variant available in ACMG multit gene analysis)
    }
