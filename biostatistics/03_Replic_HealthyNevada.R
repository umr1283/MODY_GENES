## Healthy Nevada Project Replication on MODY ##
## run 09/03/2020 ## 
...
#### Read raw data ----
input <- "/HealthyNevadaProject/HNP_carriers_8genes_EXP-085-v3_ht322_20200306.tsv"
dta <- read_tsv(
  file = input, 
  col_names = TRUE, 
  col_types = cols(
    'pipe'= col_character(), 
    'ADMIX_AFR'= col_double(),  
    'ADMIX_AMR'= col_double(),  
    'ADMIX_EAS'= col_double(),  
    'ADMIX_EUR'= col_double(),  
    'ADMIX_SAS'= col_double(),  
    'ASSAY_VERSION'= col_character(), 
    'SEX'= col_character(), 
    'birth_year'= col_character(), 
    'HNP_T2D_case'= col_double(),  
    'HNP_T2D_control'= col_double(),  
    'number_of_P_LP_variants'= col_double(), 
    'genes_mutated'= col_character(),
    'hgvsc_present'= col_character(),
    'hgvsp_present'= col_character(),
    'n_P_LP'= col_double(), 
    'number_of_diagnoses'= col_double(), 
    'number_of_E11'= col_double(), 
    'number_of_E10'= col_double(), 
    'number_of_R73'= col_double(), 
    'number_of_O24'= col_double(), 
    'number_of_250_00'= col_double(), 
    'number_of_250_01'= col_double(), 
    'number_of_vitals'= col_double(), 
    'median_BMI'= col_double(),  
    'median_contact_year'= col_double(),  
    'number_of_glucose_tests'= col_double(), 
    'number_of_glucose_tests_at_age_40plus'= col_double(), 
    'number_of_glucose_test_above_7mmolL'= col_double(), 
    'median_glucose_results_at_age_40plus'= col_double(),  
    'number_of_prescriptions'= col_double(), 
    'number_of_antidiabetic_prescriptions'= col_double(), 
...
dim(dta) ## 5742 * 45 ## 46 col now with ancestry
head(dta)
covariates_names <- c("age_now", "SEX", "median_BMI", "PC1", "PC2", "PC3", "PC4", "PC5")
var_of_interest <- c("ID_LINE", "HNP_T2D_case", "HNP_T2D_control", covariates_names, "hgvsc_present", "number_of_P_LP_variants")
...
if (!file.exists(paste0(output_directory, "/DATA_QCed.rds"))) {
  ...
  dta$is_female <- ifelse(dta$SEX=="F", TRUE, FALSE)
  
  #### Format ----
  var_of_interest <- c(var_of_interest, "genes_list", "is_female")
  ...
  ALL_DATA <- ALL_DATA %>% 
    mutate(
      T2D_status = map2_dbl(.x = HNP_T2D_case, .y = HNP_T2D_control, .f = function(.x, .y){
        status <- NA
        if (.x == 1) {status <- 1}
        if (.y == 1) {status <- 0}
        return(status)
  ...
  covariates_names <- gsub("SEX", "is_female", covariates_names)
  DATA_QCed <- ALL_DATA %>% 
