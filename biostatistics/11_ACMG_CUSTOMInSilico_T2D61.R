...
OutDir <- ".//Data/40_ACMG_CUSTOMInSilico_T2D61/" 
dir.create(
  path = OutDir,
  recursive = TRUE,
  showWarnings = FALSE,
...
CustomFile <- ".//Docs/CUSTOM_files/CUSTOM-PP3.xlsx"
InfoCUSTOM <- read_excel(path = CustomFile, sheet = 1, col_names = TRUE, col_types = "text") %>%  
  dplyr::filter(CUSTOM %in% "1") %>%  
  dplyr::select(gene, Variant) %>% 
  dplyr::rename("CUSTOM" = "Variant", "Gene" = "gene")
newcustomfile <- ".//Docs/CUSTOM_files/custom_22072020.xlsx"
writexl::write_xlsx(x = InfoCUSTOM, path = newcustomfile, col_names = TRUE)
...
liste_TR <-  liste_TR %>% 
  dplyr::filter(gene_name %in% unique(InfoCUSTOM$Gene)) ##--here only focus on 8 actionable genes 
...
for (iline in 1:nrow(liste_TR)) {
  ...
  if (!file.exists(dataset_file)) {
    ...
    if (Params["y_name"] == "Zscore") {
      Phenotypes$Zscore <- ifelse(
        test = Phenotypes$selectInd == "child",
        yes = BMIZscore(bmi = Phenotypes$BMI, age = Phenotypes$AGE, gender = Phenotypes$SEX),
        no = NA
      )
    }
    ...
    if (QCdone) {
      ...
      if (file.exists(CustomFile) & Params["analyse_LARGE_STRICT"] != "LARGE") {
        ...
        InfoCUSTOM <- InfoCUSTOM %>%
          filter(Gene == Params["Gene_Symbol"]) 
