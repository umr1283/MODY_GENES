...
## "ACMG-CUSTOM"  
## CUSTOM analyses of 33 GENES DU DIAB MONOGENIQUE
...
## To perform  the "ACMG-CUSTMOM" analyses, 
## start form STRICT QC and removed other variant in QC11 based on CUSTOM FILE ! 
y_name <- "CC_T2D61" 
binary <- TRUE
SNP_INDEL <- "BOTH"
UseCustomFile <- TRUE
...
OutDir <- ".//Data/01_ACMG_CUSTOM/" ## folder to check res "OutDirCUSTOM" in script 18
dir.create(
  path = OutDir,
  recursive = TRUE,
  showWarnings = FALSE,
...
file.copy(from = CustomFile, to = paste0(OutDir, basename(CustomFile)))
...
liste_TR <- read.table(
  ...
  filter(!gene_name %in% c("BOLA2", "BOLA2B", "DTX2P1-UPK3BP1-PMS2P11", "RMST")) ## no data
  ...
  for (iline in 1:nrow(liste_TR)) {
    ...
    if (!file.exists(dataset_file)) {
      ...
      if (SNP_INDEL == "BOTH") {
        ...
        QCdone <- TRUE
        message(paste("[MONO-GENE]", "QC Done"))
      } else {
        if (SNP_INDEL %in% c("SNP", "INDEL")) {
          ...
          if (ncol(all_genotype_data) > 0) {
            ...
            QCdone <- FALSE
            message(paste("[MONO-GENE]", "QC IMPOSSIBLE"))
      ...
      ## 1 line = 1 variant freq with 1 model (Param)
