...
setClass(
  ...
  # validity = function (object) {
    # cat("**** validity MODY_Parameters <empty> ****\n")
    # return(TRUE)
  # }
)
...
### Constructor ###
setGeneric(name = "new.MODY_Parameters", def = function (FolderOut, GroupeGene, Gene_Symbol, ENST, analyse_LARGE_STRICT, threshold, ExludeSynonymous, cluster, y_name, binary, SNP_INDEL) {standardGeneric("new.MODY_Parameters")})
setMethod(f = "new.MODY_Parameters", signature = c("missing", "missing", "missing", "missing", "missing", "missing", "missing", "missing", "missing", "missing", "missing"), definition = function (FolderOut, GroupeGene, Gene_Symbol, ENST, analyse_LARGE_STRICT, threshold, ExludeSynonymous, cluster, y_name, binary, SNP_INDEL) {new("MODY_Parameters")})
setMethod(f = "new.MODY_Parameters", signature = c("ANY", "ANY", "ANY", "ANY", "ANY", "ANY", "ANY", "ANY", "ANY", "ANY", "ANY"), definition = function (FolderOut, GroupeGene, Gene_Symbol, ENST, analyse_LARGE_STRICT, threshold, ExludeSynonymous, cluster, y_name, binary, SNP_INDEL) {
  ...
  return(new("MODY_Parameters", FolderOut = FolderOut, GroupeGene = GroupeGene, Gene_Symbol = Gene_Symbol, ENST = ENST, analyse_LARGE_STRICT = analyse_LARGE_STRICT, threshold = threshold, ExludeSynonymous = ExludeSynonymous, cluster = cluster, y_name = y_name, binary = binary, SNP_INDEL = SNP_INDEL))
})


### Is ###
setGeneric(name = "is.MODY_Parameters", def = function (object) {standardGeneric("is.MODY_Parameters")})
setMethod(f = "is.MODY_Parameters", signature = "ANY", definition = function (object) {
  if (length(object)>1) {
    return(sapply(object, is.MODY_Parameters))
  } else {
    if (class(object) == "MODY_Parameters") {
      return(TRUE)
    } else {
      return(FALSE)
    }
...
### Show ###
setMethod(f = "show", signature = "MODY_Parameters", definition = function (object){
...
### Getteur ###
setMethod(f = "[", signature = "MODY_Parameters", definition = function (x, i, j, drop){
  switch(EXPR = i, 
    "FolderOut" = {
      if (missing(j)) {
        return(x@FolderOut)
        ...
        if (j>length(x@FolderOut)) {
          stop("[MODY_Parameters:get] indice out of limits")
        } else {
    ...
    stop("[MODY_Parameters:get] ", i, " is not a \"MODY_Parameters\" slot")
  )
})


### Setteur ###
setMethod(f = "[<-", signature = "MODY_Parameters", definition = function (x, i, j, value){
    switch(EXPR = i, 
    "FolderOut" = {
      if (missing(j)) {
        x@FolderOut <- value
        ...
        if (j>length(x@FolderOut)) {
          stop("[MODY_Parameters:set] indice out of limits")
        } else {
          x@FolderOut[j] <- value
        }
      }
    ...
    stop("[MODY_Parameters:set] ", i, " is not a \"MODY_Parameters\" slot")
  )
  validObject(x)
  return(invisible(x))
})
...
### Summary ###
setMethod(f = "summary", signature = "MODY_Parameters", definition = function (object){
  if (missing(object)){
    stop("[MODY_Parameters:summary] \"object\" is missing", call. = FALSE)
    return(invisible())
  } else {}
  warning("[MODY_Parameters:summary] No summary method defined for \"MODY_Parameters\" object!", call. = FALSE)
  return(invisible(object))
})
