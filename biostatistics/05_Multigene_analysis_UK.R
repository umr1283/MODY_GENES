## Do multigene analysis in UK bio bank ##
...
## input 
input_directory <- paste(working_directory, "02_Replic_UKBioBank", sep = "/")
...
y_name <- "T2D_status"
binary <- TRUE
covar_names <- c("age_attended_enrollment", "is_female", "BMI_21001", "PC1", "PC2", "PC3", "PC4", "PC5")
...
saveRDS(object = stat_MiST, paste0(output_directory, "/stat_MiST.RDS"))
saveRDS(object = stat_rare, paste0(output_directory, "/stat_rare.RDS"))
...
writexl::write_xlsx(
  ...
  path = paste0(output_directory, "/multigene_UK.xlsx"), 
  col_names = TRUE
)
message("xlsx writen ! ")
