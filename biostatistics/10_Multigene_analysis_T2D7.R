...
## nohup nice -15 Rscript 10_Multigene_analysis_T2D7.R CLASSIQUE &
## nohup nice -15 Rscript 10_Multigene_analysis_T2D7.R ACMGCUSTOM &
...
script_name <- "10_Multigene_analysis_T2D7"
project_directory <- "./"
...
if (y_name %in% c("CC_OBESITE", "CC_Obesity")) {
...
if (VERSION == "ACMGCUSTOM") {
  analyses_to_do <- c("STRICT")  ## start from strict analyses
}
...
for (analyse_LARGE_STRICT in analyses_to_do) {
  ...
  for (igroup in 1:nrow(everything_qcs)) {
    ...
    ##--here adapt for ACMGCUSTOM VERSION 
    message(VERSION)
    if (VERSION == "ACMGCUSTOM") {
      message("QC12 based on custom file")
      
      QC12_customfile <- read_excel(
        path = ".//Docs/CUSTOM_files/custom_21072020.xlsx",  
        sheet = 1,
        col_names = TRUE, 
        col_types = "text"
      )
